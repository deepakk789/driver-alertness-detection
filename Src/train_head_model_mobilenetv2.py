"""
MobileNetV2 Transfer Learning — Head Pose Model Comparison
=====================================================
This script trains TWO MobileNetV2 variants for head pose classification
(LOOKING_FORWARD vs LOOKING_AWAY) and compares them against the custom CNN baseline.
...
TRAIN_DIR = r"D:\project folder\DRIVER ALLERTNESS DETECTION SYSTEM\DATASET_COMBINED\TRAIN\HEAD_TRAIN"
VAL_DIR   = r"D:\project folder\DRIVER ALLERTNESS DETECTION SYSTEM\DATASET_COMBINED\VALIDATION\HEAD_VAL"
TEST_DIR  = r"D:\project folder\DRIVER ALLERTNESS DETECTION SYSTEM\DATASET_COMBINED\TEST\HEAD_TEST"
...
frozen_callbacks = [
    ...
    ModelCheckpoint("Models/head_model_mobilenet_frozen.h5", save_best_only=True)
]
...
tuned_callbacks = [
    EarlyStopping(patience=5, restore_best_weights=True),
    ModelCheckpoint("Models/head_model_mobilenet_tuned.h5", save_best_only=True)
]
...
try:
    custom_model = load_model("Models/head_model.h5")
    custom_loss, custom_acc = custom_model.evaluate(test_data)
except Exception as e:
    print(f"Error loading custom CNN baseline: {e}")
    custom_loss, custom_acc = 0.0, 0.0
...
# ===========================
print("\n" + "="*60)
print("HEAD POSE MODEL COMPARISON RESULTS")
print("="*60)
